 ...
 */
// var spaceLayout = document.getElementById('space');
// var space = spaceLayout.getContext("2d");
// var spaceInfo = {};
// spaceInfo.centerPoint = {x:0,y:0};

/*构建宇宙*/
function bigBoom() {

    space.translate(spaceLayout.width / 2, spaceLayout.height / 2);
    spaceLayout.style.backgroundColor = '#000000';
    spaceLayout.style.width = '100%';
    spaceLayout.style.height = '100%';
}
...
function flashCenter() {
    ...
    setInterval(function () {
        space.clearRect(-6, -6, 12, 12);
        space.beginPath();
        space.arc(spaceInfo.centerPoint.x, spaceInfo.centerPoint.y, growRadius, 0, 2 * Math.PI);
        space.fillStyle = '#842B00';
        space.strokeStyle = '#F0F0F0';
        space.stroke();
        if (growRadius === 5) {
            growRadius = 1;
        } else {
            growRadius = growRadius + 0.5;
        }
    }, 300);
}
/* 不重叠位置 */
 ...
 * grid:纵行栅格
 */
var targets = {table: [], sphere: [],sphere1:[], helix: [], grid: []};
...
function init() {
     ...
     * @param rowNo 行号
     */
    function layoutWithTable(colNo, rowNo) {
        var object = new THREE.Object3D();
        object.position.x = ( colNo * 140 ) - 1330;
        object.position.y = -( rowNo * 180 ) + 990;
        //为表格布局添加位置元素
    ...
    for (var i = 0; i < table.length; i += 5) {
        ...
        layoutWithTable(table[i + 3], table[i + 4]);
    }

    // sphere

    var vector = new THREE.Vector3();
    var sphereRadius = 800;
    ...
    //button example
    function tweenElement(targets,duration) {
        for (var i = 0; i < objects.length; i++) {
    ...
    button.addEventListener('click', function (event) {
        ...
        //
        //
        // tweenElement(targets.sphere,2000);
        // tweenElement(targets.sphere1,2000);
        //
